...
test.describe.serial('Guest Session', () => {
  ...
  test('Do not authenticate as guest user when an existing non-guest session is active', async ({ adaContext }) => {
...
test.describe.serial('Login and Registration', () => {
  ...
  test('Log into account that exists', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('Meow...')).toBeVisible()
  })
  ...
  test('Do not force create a guest session if non-guest session already exists', async ({ page }) => {
    await authPage.login(testUser.email, testUser.password)
    await page.waitForURL('/')
...
test.describe('Entitlements', () => {
  ...
  test('Guest user cannot send more than 20 messages/day', async () => {
    ...
    await chatPage.sendUserMessage('Why is the sky blue?')
    await chatPage.expectToastToContain('You have exceeded your maximum number of messages for the day! Please try again later.')
  })
})
