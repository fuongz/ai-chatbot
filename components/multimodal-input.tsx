...
function PureMultimodalInput({
  ...
  return (
    <div className="relative w-full flex flex-col gap-4">
      ...
      {messages.length === 0 && attachments.length === 0 && uploadQueue.length === 0 && <SuggestedActions append={append} chatId={chatId} selectedVisibilityType={selectedVisibilityType} />}
      ...
      <Textarea
        ...
        onKeyDown={(event) => {
          if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) {
            event.preventDefault()
            if (status !== 'ready') {
              toast.error('Vui lòng chờ cho Mây trả lời xong nha!')
            } else {
              submitForm()
            }
          }
